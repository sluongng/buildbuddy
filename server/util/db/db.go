...
func (dbh *DBHandle) TransactionWithOptions(ctx context.Context, opts *Options, txn txRunner) error {
	...
	if opts.queryName != "" {
		db.Statement.Settings.Store(gormQueryNameKey, opts.queryName)
	}
	return db.WithContext(ctx).Transaction(txn)
}
...
func instrumentGORM(gdb *gorm.DB) {
	...
	recordMetrics := func(db *gorm.DB) {
		...
		qv, _ := db.Statement.Settings.LoadAndDelete(gormQueryNameKey)
